...
GEMINI_MATCH_WORKERS    = 3   # parallel Gemini match calls (per course)
# ─────────────────────────────────────────────────────────────────────────────
...
def canvas_get(path: str, params: dict = None) -> any:
    ...
    _session.headers.update({"Authorization": f"Bearer {CANVAS_API_TOKEN}"})
...
def _try_download_linked_pdf(html: str, dest_dir: str, asgn_name: str) -> str | None:
    ...
    for raw_url in dict.fromkeys(urls):
        if "/courses/" in raw_url and "/files/" not in raw_url and "download" not in raw_url:
            continue
        try:
            resp = _session.head(raw_url, allow_redirects=True, timeout=10)
            ct = resp.headers.get("Content-Type", "")
            final_url = resp.url
            is_pdf = ".pdf" in final_url.lower() or "application/pdf" in ct
            if not is_pdf:
                r2 = _session.get(raw_url, stream=True, timeout=15)
...
def upload_to_gemini(file_paths: list[str]) -> dict[str, any]:
    """Upload all files to Gemini sequentially (parallel causes SSL errors)."""
    uploaded = {}
...
def match_all_assignments(model, course_uploads: dict, assignments: list[dict]) -> list[dict]:
    """Run all assignment matches for a course in parallel."""
    results = [None] * len(assignments)

    def _run(idx: int, asgn: dict):
        aname = asgn["name"]
        apath = asgn["local_path"]
        print(f"    ⏳ Matching: {aname}")
        try:
            asgn_upload = genai.upload_file(apath)
            return idx, match_assignment(model, course_uploads, asgn_upload, aname)
        except json.JSONDecodeError:
            print(f"    ❌ Non-JSON response for '{aname}' — skipping.")
        except Exception as e:
            print(f"    ❌ Error matching '{aname}': {e}")
        return idx, None

    with ThreadPoolExecutor(max_workers=GEMINI_MATCH_WORKERS) as ex:
        futures = {ex.submit(_run, i, a): i for i, a in enumerate(assignments)}
        for fut in as_completed(futures):
